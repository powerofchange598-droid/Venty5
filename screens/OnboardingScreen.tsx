...
const PersonalOnboardingForm: React.FC<{ onNext: (data: Omit<OnboardingData, 'accountPlan' | 'primarySpendingCategory'>) => void; accountPlan: 'single' | 'family'; initialData?: { name: string; email: string } | null }> = ({ onNext, accountPlan, initialData }) => {
    ...
    const handleFinish = async () => {
        ...
        if (!isSocialSignup) {
            const signupFunc = onSignUp || signUpWithEmail;
            const created = await signupFunc(email, password);
            if (!created) {
...
const OnboardingScreen: React.FC<OnboardingScreenProps> = ({ onComplete, onJoinFamily, onExploreAsGuest, onSignUp }) => {
